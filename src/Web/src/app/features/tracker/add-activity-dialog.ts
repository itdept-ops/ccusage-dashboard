...
import { ActivityCalorieMode, UnitSystem, UpsertActivityRequest, WatchActivityDto } from '../../core/models';
...
@Component({
  ...
  imports: [
    FormsModule, MatDialogModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatButtonToggleModule,
  ],
  template: `
    ...
    <mat-dialog-content class="ac-body">
      <mat-form-field appearance="outline" class="ac-field">
        ...
        <input matInput type="number" min="0" step="1" inputmode="numeric" cdkFocusInitial
      ...
      <mat-form-field appearance="outline" class="ac-field">
        ...
        <input matInput type="number" min="0" step="0.1" inputmode="decimal"
               [ngModel]="distanceDisp()" (ngModelChange)="distanceDisp.set($event)" />
      ...
      <div class="ac-mode">
        ...
        <mat-button-toggle-group [value]="calorieMode()" (change)="calorieMode.set($event.value)"
                                 aria-label="How active calories count" class="ac-mode-toggle">
  ...
  styles: `
    ...
    .ac-body { min-width: min(340px, 80vw); padding-top: 4px !important;
      display: flex; flex-direction: column; gap: var(--tech-space-2); }
    .ac-field { width: 100%; }
    .ac-mode { display: flex; flex-direction: column; gap: 6px; }
    .ac-mode-toggle { width: 100%; }
    .ac-mode-toggle .mat-button-toggle { flex: 1 1 0; }
    .ac-mode-note { margin: 0; font-size: var(--tech-fs-label); color: var(--tech-text-tertiary); }
    .ac-actions { padding: var(--tech-space-3) var(--tech-space-4); gap: 8px;
      button { border-radius: var(--tech-r-control); font-weight: 600; min-height: 44px; } }
    .ac-clear { margin-right: auto; color: var(--tech-error); }
  `,
})
export class AddActivityDialog {
  ...
  readonly distanceDisp = signal<number | null>(this.toDistDisp(this.data.activity?.distanceMeters));
  ...
  private toDistDisp(m: number | null | undefined): number | null {
    ...
    return this.imperial ? Math.round(metersToMiles(m) * 10) / 10 : Math.round((m / 1000) * 10) / 10;
  }
