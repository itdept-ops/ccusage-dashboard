  ...
  template: `
    ...
    <mat-dialog-content class="md-body">
      ...
      <mat-form-field appearance="outline" class="md-field">
        ...
        <input matInput type="date" cdkFocusInitial [ngModel]="toDate()" (ngModelChange)="toDate.set($event)" />
      ...
      <fieldset class="md-cats">
        ...
        @for (c of categories; track c.key) {
          <mat-checkbox class="md-cat" [checked]="isChecked(c.key)" (change)="toggle(c.key, $event.checked)">
    ...
    <mat-dialog-actions class="md-actions" align="end">
      ...
      <button mat-flat-button type="button" color="primary" [disabled]="!canMove()" (click)="move()">Move</button>
    </mat-dialog-actions>
  `,
  styles: `
    .md-title { font-family: var(--tech-font-ui); font-weight: 700; color: var(--tech-text); }
    .md-body { display: flex; flex-direction: column; gap: var(--tech-space-3);
      min-width: min(340px, 82vw); padding-top: 4px !important; }
    .md-intro { margin: 0; color: var(--tech-text-dim); font-size: .9rem; line-height: 1.4; }
    .md-field { width: 100%; }
    .md-cats { display: flex; flex-direction: column; gap: var(--tech-space-1);
      border: none; margin: 0; padding: 0; }
    .md-cats legend { margin-bottom: var(--tech-space-1); }
    .md-cat { min-height: 40px; }
    .md-cat-icon { font-size: 18px; height: 18px; width: 18px; vertical-align: text-bottom;
      margin-right: 2px; color: var(--tech-text-dim); }
    .md-note, .md-warn { display: flex; align-items: flex-start; gap: 6px; margin: 0;
      font-size: .82rem; line-height: 1.35; }
    .md-note { color: var(--tech-text-dim); }
    .md-note mat-icon, .md-warn mat-icon { font-size: 18px; height: 18px; width: 18px; flex: none; }
    .md-warn { color: var(--tech-error, #ff5c6c); }
    .md-actions { padding: var(--tech-space-3) var(--tech-space-4); gap: 8px;
...
export class MoveDayDialog {
  ...
  readonly canMove = computed(() => !!this.toDate() && !this.sameDate() && this.selected().size > 0);
