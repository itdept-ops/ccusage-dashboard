...
import { CalendarDay, HeatmapCell, SummaryBucket, UsageFilter, UsageStats } from '../../core/models';
...
@Component({
  ...
  imports: [
    CommonModule, MatCardModule, MatButtonToggleModule, MatProgressBarModule, MatIconModule,
    MatButtonModule, MatDialogModule, ChartComponent,
  ],
...
export class Calendar {
  ...
  constructor() {
    ...
    this.api.stats().subscribe({ next: s => this.metrics.set(s), error: () => { /* non-critical */ } });
    this.api.heatmap().subscribe({ next: h => this.heat.set(h), error: () => { /* non-critical */ } });
  ...
  shortId(id: string): string { return id.length > 18 ? id.slice(0, 8) + '…' + id.slice(-6) : id; }

  readonly heatmapChart = computed<EChartsOption>(() => {
    const cells = this.heat();
    if (!cells.length) return { title: { text: 'No data', left: 'center', top: 'center', textStyle: { color: '#5e6c82' } } };
    const dayNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    const hours = Array.from({ length: 24 }, (_, h) => `${h}`);
    const max = Math.max(...cells.map(c => c.count));
    return {
      tooltip: { formatter: (p: any) => `${dayNames[p.value[1]]} ${this.fmtHour(p.value[0])}<br/>${p.value[2]} messages` },
      grid: { left: 44, right: 16, top: 8, bottom: 58 },
      xAxis: { type: 'category', data: hours, splitArea: { show: true }, axisLabel: { interval: 1, color: '#5e6c82', fontFamily: 'JetBrains Mono, monospace', fontSize: 10 } },
      yAxis: { type: 'category', data: dayNames, splitArea: { show: true }, axisLabel: { color: '#7286a0', fontFamily: 'JetBrains Mono, monospace', fontSize: 11 } },
      visualMap: { min: 0, max, calculable: true, orient: 'horizontal', left: 'center', bottom: 6, itemWidth: 12, itemHeight: 120, textStyle: { color: '#7286a0', fontSize: 11 }, inRange: { color: ['#15233a', '#1d4e7a', '#3d8bff', '#7c8bff', '#f472b6'] } },
      series: [{ type: 'heatmap', data: cells.map(c => [c.hour, c.day, c.count]), itemStyle: { borderColor: '#0b0f17', borderWidth: 1 }, label: { show: false } }],
    };
  });
