...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="ac-body">
      @if (loading()) {
        ...
        <button
          type="button"
          class="ac-row ac-row--clear"
          (click)="pick(null)"
          [class.ac-row--active]="data.currentUserId == null"
        >
          <span class="ac-avatar ac-avatar--clear"
            ><mat-icon aria-hidden="true">person_off</mat-icon></span
          >
          <span class="ac-name">No one (open for claiming)</span>
          @if (data.currentUserId == null) {
            <mat-icon class="ac-check" aria-hidden="true">check</mat-icon>
          }
        </button>
        ...
        @for (c of contacts(); track c.userId) {
          ...
          >
            ...
            @if (c.userId === data.currentUserId) {
              <mat-icon class="ac-check" aria-hidden="true">check</mat-icon>
            }
          </button>
        }
  ...
  styles: `
    ...
    .ac-row {
      ...
      min-height: 48px;
      &:hover {
        background: var(--tech-panel-2, rgba(255, 255, 255, 0.04));
      }
      &--active {
        border-color: var(--tech-accent, #3fd8d0);
      }
      &--clear {
    ...
    .ac-avatar {
      ...
      align-items: center;
      justify-content: center;
      background: var(--tech-panel-2, #1b2230);
      font-size: 0.8rem;
      font-weight: 700;
      &--init {
