...
import {
  IdentityAutoSignal,
  IdentityImportPreview,
  IdentityMapData,
  IdentityProposedTime,
  IdentityRole,
  IdentityRoleTotal,
} from '../../core/models';
...
export class FamilyIdentityMap {
  ...
  readonly chartAria = computed<string>(() => {
    ...
    const top = rows
      .slice(0, 3)
      .map((r) => `${r.roleName} ${r.pct}%`)
      .join(', ');
    return (
      `Time split across ${rows.length} role${rows.length === 1 ? '' : 's'}, ` +
      `${this.minutesLabel(this.totalMinutes())} total. Top: ${top}.`
    );
  });

  /** True if any import row has a role assigned (the Commit button is enabled). */
  readonly canCommitImport = computed<boolean>(() =>
    this.importRows().some((r) => r.roleId != null),
  );
  ...
  onCustomRange(): void {
    ...
    if (this.fromDate() && this.toDate() && this.fromDate() > this.toDate()) {
      this.snack.open('The start date must be on or before the end date.', 'OK', {
        duration: 4000,
      });
      return;
    }
  ...
  async addRole(): Promise<void> {
    ...
    try {
      ...
      const i =
        (FamilyIdentityMap.PALETTE.indexOf(this.newRoleColor()) + 1) %
        FamilyIdentityMap.PALETTE.length;
      this.newRoleColor.set(FamilyIdentityMap.PALETTE[i]);
      await this.refresh();
      if (this.logRoleId() == null) this.logRoleId.set(this.activeRoles()[0]?.id ?? null);
    } catch (e) {
      this.snack.open(
        this.messageOf(e, "Couldn't add that role — is the name already used?"),
        'OK',
        { duration: 4000 },
      );
    } finally {
  ...
  async renameRole(role: IdentityRole, name: string): Promise<void> {
    ...
    try {
      ...
      this.snack.open(
        this.messageOf(e, "Couldn't rename that role — is the name already used?"),
        'OK',
        { duration: 4000 },
      );
    }
  ...
  async deleteRole(role: IdentityRole): Promise<void> {
    const ok = await this.confirm({
      ...
      message:
        'This removes the role and ALL time logged against it (and any classification rules that point ' +
    ...
    try {
      ...
      this.snack.open("Couldn't delete that role just now. Please try again.", 'OK', {
        duration: 4000,
      });
    }
  }
  ...
  async runPreview(): Promise<void> {
    ...
    try {
      const p = await firstValueFrom(
        this.api.identityImportPreview(this.importFrom(), this.importTo()),
      );
      this.preview.set(p);
      if (p.notReady) {
        this.importRows.set([]);
        this.snack.open(
          'Connect your Google Calendar (in the Family Calendar) to import time.',
          'OK',
          { duration: 5000 },
        );
        return;
      ...
      this.snack.open(
        this.messageOf(e, "Couldn't read your calendar just now. Please try again."),
        'OK',
        { duration: 4000 },
      );
    } finally {
  ...
  toggleRemember(row: ImportRow): void {
    this.importRows.update((rows) =>
      rows.map((r) => (r === row ? { ...r, remember: !r.remember } : r)),
    );
  }
  ...
  async commitImport(): Promise<void> {
    ...
    if (rows.length === 0) {
      this.snack.open('Assign a role to at least one event to import it.', 'OK', {
        duration: 3500,
      });
      return;
    }
    ...
    try {
      ...
      this.snack.open(
        this.messageOf(e, "Couldn't import those just now. Please try again."),
        'OK',
        { duration: 4000 },
      );
    } finally {
  ...
  async refreshAuto(): Promise<void> {
    ...
    try {
      const res = await firstValueFrom(
        this.api.identityAutoSuggest(this.fromDate(), this.toDate()),
      );
      this.autoRows.set(
        (res.signals ?? []).map((signal) => ({
          signal,
          // The server returns 0 for "no match"; treat that as unassigned (null) so the user picks.
          roleId:
            signal.suggestedRoleId && signal.suggestedRoleId > 0 ? signal.suggestedRoleId : null,
        })),
      );
      this.autoLoaded.set(true);
    } catch (e) {
      this.snack.open(
        this.messageOf(e, "Couldn't read your recent activity just now. Please try again."),
        'OK',
        { duration: 4000 },
      );
    } finally {
  ...
  async applyAuto(): Promise<void> {
    ...
    if (rows.length === 0) {
      this.snack.open('Assign a role to at least one activity to apply it.', 'OK', {
        duration: 3500,
      });
      return;
    }
