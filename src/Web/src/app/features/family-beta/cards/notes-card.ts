...
@Component({
  ...
  `,
  styles: [`
    /* --wa / --wb inherited from HearthShell's .w element via the [style.--wa] binding. */
    .badge {
      margin-left: auto; font-size: 12px; font-weight: 800; min-width: 22px; text-align: center;
      padding: 3px 9px; border-radius: var(--r-pill);
      background: color-mix(in srgb, var(--wa, #f59e0b) 22%, transparent);
      color: color-mix(in srgb, var(--wa, #fcd34d) 90%, var(--ink));
    }
    ...
    .note {
      ...
      background: color-mix(in srgb, var(--wa, #f59e0b) 7%, var(--bg-sink)); border: 1px solid var(--hairline);
      transition: transform 120ms var(--ease-spring), box-shadow 120ms var(--ease-out), background 120ms var(--ease-out);
    }
    @media (hover: hover) {
      .note:hover {
        transform: translateY(-1px);
        box-shadow: 0 4px 14px color-mix(in srgb, var(--wa, #f59e0b) 18%, transparent);
        background: color-mix(in srgb, var(--wa, #f59e0b) 12%, var(--bg-sink));
      }
    }
    .note:active { transform: scale(.99); }
    .note:focus-visible {
      outline: 2px solid color-mix(in srgb, var(--wa, #f59e0b) 90%, transparent);
      outline-offset: 2px;
    }
    .note__pin {
    ...
    .note__pin mat-icon {
      color: color-mix(in srgb, var(--wa, #fcd34d) 85%, var(--ink));
      font-size: 18px; width: 18px; height: 18px;
    }
    .note__title { flex: 1 1 auto; font-size: 15px; font-weight: 600; color: var(--ink); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
