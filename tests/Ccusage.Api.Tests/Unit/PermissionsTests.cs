...
public class PermissionsTests
{
    ...
    [Fact]
    public void RecipesUse_GroceryUse_and_MealsUse_are_Tools_non_ai_and_not_defaultable()
    {
        // All gate private, owner/household-scoped data, so they are deliberate grants in the "Tools" group.
        ...
        Permissions.Catalog.Single(p => p.Key == Permissions.MealsUse).Group.Should().Be("Tools");
        Permissions.IsAi(Permissions.RecipesUse).Should().BeFalse();
        Permissions.IsAi(Permissions.GroceryUse).Should().BeFalse();
        Permissions.IsAi(Permissions.MealsUse).Should().BeFalse();
        ...
        Permissions.IsDefaultable(Permissions.MealsUse).Should().BeFalse();
        Permissions.All.Should().Contain(Permissions.RecipesUse).And.Contain(Permissions.GroceryUse)
            .And.Contain(Permissions.MealsUse);
        // A page gate (like grocery/recipes), so it must NOT appear in the *.view set.
        Permissions.Views.Should().NotContain(Permissions.MealsUse);
    }
